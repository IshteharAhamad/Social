  ...
  try {
    ...
    if (password === newPassword) {
      res
        ...
        .json({
          success: false,
          message: "Current password and change password should not match!",
        });
    }
    // const ValidatePassword=await bcrypt.compare(password,user.password);
...
const sendEmailToUser = async (req, res) => {
  try {
    ...
    const content = {
      tomail:email,
      subject: "Information sent",
      body: "<h1>Welcome to being here</h1>",
    };
   // Send the email using the sendmail function
    const emailDetails = await sendmail(content.tomail, content.subject, content.body);
    res.json({
      success: true,
      emailDetails,
